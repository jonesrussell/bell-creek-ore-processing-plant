...
SVGjsAnim.prototype.setupReceiving = function () {
  ...
  var heading = this.draw
    .heading("RECEIVING", this.wistiaObjs.receiving)
    .move(355, 50);
  ...
  this.rockBreaker = this.draw.rockBreaker({
    x: this.rockBreakerX,
    y: this.rockBreakerY,
    scale: this.rockBreakerScale,
  });
  step.content(this.rockBreaker);

  // Rock Breaker Ore Pile
  var orePileRockBreaker = this.draw
    .image("images/ore/ore_pile_2.svg", 86, 37)
    .move(705, 400)
    .scale(0.8);
  ...
  bulletsGroup.add(this.draw.bullets(bullets, 300));
